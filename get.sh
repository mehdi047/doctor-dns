...
#
# All this does is fetch install.sh, check it arrived whole, and hand over to
# it. Everything that matters happens in install.sh, which is the file worth
# reading before running any of this:
#
#   https://github.com/mehdi047/doctor-dns/blob/main/install.sh
#
...
# A truncated download is the failure worth catching here. install.sh carries
# every config it writes in its own tail, so half of it is still a runnable
# script - one that would set up a machine with pieces missing. Three cheap
# checks, and none of them can be true of a partial file.
size=$(wc -c < "$tmp")
...
mkdir -p "$DEST"
install -m 755 "$tmp" "$DEST/install.sh"

say "installer saved to $DEST/install.sh"
if command -v sha256sum >/dev/null 2>&1; then
    ...
    printf '    sha256  %s\n' "$(sha256sum < "$DEST/install.sh" | cut -d' ' -f1)"
fi

# install.sh asks which side of the service this machine is, the address of the
# other one, and a password. Piped into sh, this script's stdin is the pipe, so
# every one of those prompts would read end-of-file and the install would run
# on answers nobody gave. Hand it the terminal instead.
if [ -t 0 ]; then
